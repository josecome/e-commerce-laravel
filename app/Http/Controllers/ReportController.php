...
class ReportController extends Controller
{
    function getCategories(Request $req)
    {
        ...
        switch (true) {
            ...
            case $typeOfreq == 'categories_with_date_between':
                $data = DB::table('prod_categories')->select('*')->whereBetween(
                    'created_at',
    ...
    function getProducts(Request $req)
    {
        ...
        switch (true) {
            ...
            case $typeOfreq == 'product_with_price_between':
                $data = DB::table('products')->select('*')->whereBetween(
                    'created_at',
                    [$initial_date, $last_date]
                )->get();
                break;
            case $typeOfreq == 'products_with_price_more_than':
                $data = DB::table('cart')->select('*')->where(
                    [['created_at', '>', $initial_price]]
                )->get();
                break;
            case $typeOfreq == 'products_with_price_less_than':
                $data = DB::table('products')->select('*')->where(
                    [['created_at', '<', $last_price]]
                )->get();
                break;
            default:
                $data = DB::table('products')->select('*')->where('deleted_at', null)->get();
    ...
    function getPurchasedProducts(Request $req)
    {
        ...
        switch (true) {
            case $typeOfreq == 'products_all_list':
                $data = DB::table('cart')->select('*')->where(
                    [['deleted_at', '=', 0], ['purchased', '=', 1]]
                )->with('user')->get();
                break;
            case $typeOfreq == 'products_with_price_more_than':
                $data = DB::table('cart')->select('*')->where(
                    [['deleted_at', '=', 0], ['price', '>', $initial_price]]
                )->with('user')->get();
                break;
            case $typeOfreq == 'products_with_price_less_than':
                $data = DB::table('cart')->select('*')->where(
                    [['deleted_at', '=', 0], ['price', '<', $last_price]]
                )->with('user')->get();
                break;
            default:
                $data = DB::table('cart')->select('*')->where(
